...
/************************************************************************************************/
	...
	if (!bGetBetterUnits && (bIsCapitalArea) && (iAreaBestFoundValue < (iMinFoundValue * 2)))
	{
		...
		else
		{
			...
			{
				if (GET_PLAYER((PlayerTypes)iI).isBarbarian() || GET_PLAYER((PlayerTypes)iI).isNative() || GET_PLAYER((PlayerTypes)iI).getCivilizationType() == CIV_CELTS)
				{
...
UnitTypes CvCityAI::AI_bestUnit(bool bAsync, AdvisorTypes eIgnoreAdvisor, UnitAITypes* peBestUnitAI)
{
	...
	case CIV_PERSIA:
		aiUnitAIVal[UNITAI_ATTACK] *= 2;
		break;
	case CIV_ROME:      // leave unit AI unchanged so far
...
BuildingTypes CvCityAI::AI_bestBuildingThreshold(int iFocusFlags, int iMaxTurns, int iMinThreshold, bool bAsync, AdvisorTypes eIgnoreAdvisor)
{
	...
	if (getCivilizationType() == CIV_MOORS && isCapital() && getRegionID() == REGION_IBERIA)
	{
...
/************************************************************************************************/
			...
			if (iPass > 0)
			{
				for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
				{
					...
					{
						if (getCivilizationType() == CIV_CHINA && iI == 2)
						{
							iTempValue += 100;
						}
