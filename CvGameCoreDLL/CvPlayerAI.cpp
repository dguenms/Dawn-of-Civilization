	...
	for (iPass = 0; iPass < 4; iPass++)
	{
		for(pLoopUnit = firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = nextUnit(&iLoop))
		{
			...
			if (bValid)
			{
				...
				if (!bNoDisband)
				{
					if (pLoopUnit->canFight() && !pLoopUnit->isFound() && !isMinorCiv()) // Leoreth: do not disband American pioneers or minor units
					{
...
DenialTypes CvPlayerAI::AI_cityTrade(CvCity* pCity, PlayerTypes ePlayer) const
{
	...
	if (isHuman() && (pCity->getNumActiveBuilding(BUILDING_PLAGUE) > 0))
	{
		return DENIAL_NO_GAIN;
	}
	//Rhye - end
	...
	if (pCity->plot()->getSettlerValue(ePlayer) == 0)
	{
		return DENIAL_NO_GAIN;
	}
		...
		if (GET_PLAYER(ePlayer).getTeam() != getTeam())
		{
			if ((pCity->plot()->calculateCulturePercent(ePlayer) == 0) && !(pCity->isEverOwned(ePlayer)) && (GET_PLAYER(ePlayer).getNumCities() > 3))
			{
				...
				*/
				...
				if (!pCity->isPreviousOwner(ePlayer))
				{
					if (pCity->plot()->getSettlerValue(ePlayer) == 0 && pCity->plot()->getWarValue(ePlayer) == 0)
					{
						return DENIAL_UNKNOWN;
					}
				}
