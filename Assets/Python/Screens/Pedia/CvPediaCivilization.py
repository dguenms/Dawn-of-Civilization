...
class CvPediaCivilization:
	def __init__(self, main):
		...
		self.Y_INFO_TEXT = self.Y_ICON + 25
		self.W_INFO_TEXT = 180
		self.H_INFO_TEXT = self.H_INFO_PANE - 20

		self.W_LEADERS = self.top.W_PEDIA_PAGE - self.W_INFO_PANE - 10
		self.H_LEADERS = 110
		self.X_LEADERS = self.X_INFO_PANE + self.W_INFO_PANE + 10
		self.Y_LEADERS = self.Y_INFO_PANE + self.H_INFO_PANE - self.H_LEADERS

		self.X_TECHS = self.top.X_PEDIA_PAGE
		self.Y_TECHS = self.Y_INFO_PANE + self.H_INFO_PANE + 10
		self.W_TECHS = self.top.W_PEDIA_PAGE / 3 - 3
		self.H_TECHS = 110

		self.X_BUILDINGS = self.X_TECHS + self.W_TECHS + 10
		self.Y_BUILDINGS = self.Y_TECHS
		self.W_BUILDINGS = self.W_TECHS
		...
		self.X_HISTORY = self.X_TECHS
		self.Y_HISTORY = self.Y_TECHS + self.H_TECHS + 10
	...
	def placeInfo(self):
		...
		szText = CivInfo.getDescription().replace(" ", "\n")
		screen.appendListBoxString(panel, u"<font=4b>" + szText + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def placeBuildings(self):
		...
		for iBuilding in xrange(gc.getNumBuildingClassInfos()):
			...
			if (iDefaultBuilding > -1 and iUniqueBuilding > -1 and iDefaultBuilding != iUniqueBuilding):
				screen.attachImageButton(panel, "", gc.getBuildingInfo(iUniqueBuilding).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_HR_BUILDING, iUniqueBuilding, -1, False)
