	...
	try:
		...
		for y in reversed(range(iWorldY)):
			...
			for x in range(iWorldX):
				...
				elif iSettlerValue > 0 and (not bForeignCore or not DISPLAY_CONTESTED): lRow.append(HISTORICAL)
				elif iSettlerValue > 0 and bForeignCore: lRow.append(CONTESTED)
				elif iSettlerValue == 0 and (not bForeignCore or not DISPLAY_FOREIGN_CORE): lRow.append(FOREIGN)
				elif iSettlerValue == 0 and bForeignCore: lRow.append(FOREIGN_CORE)
