...
class TestAreaPopulationPercent(ExtendedTestCase):
	...
	def setUp(self):
		self.area = AreaArgumentFactory().of([(57, 35)]).named("Test Area")
	...
	def test_description(self):
		self.assertEqual(self.requirement.description(), "40% of the world population in Test Area")
	
	def test_areas(self):
		self.assertEqual(self.requirement.areas(), {"Test Area": plots.of([(57, 35)])})
	...
	def test_outside(self):
		...
		try:
			self.assertEqual(self.requirement.evaluate(self.evaluator), 5)
			self.assertEqual(self.requirement.percentage(self.evaluator), 12.5)
			self.assertEqual(self.requirement.fulfilled(self.evaluator), False)
			self.assertEqual(self.requirement.progress(self.evaluator), self.FAILURE + "Population in Test Area: 12.50% / 40%")
		finally:
			cities.kill()
...
class TestPopulationInAreaPercent(ExtendedTestCase):

	def setUp(self):
		self.area = AreaArgumentFactory().of([(57, 35), (59, 35)]).named("Test Area")
		self.requirement = PopulationInAreaPercent(self.area, 40).create()
		self.goal = TestGoal()
	...
	def test_str(self):
		self.assertEqual(str(self.requirement), "PopulationInAreaPercent(Test Area, 40%)")
	
	def test_repr(self):
		self.assertEqual(repr(self.requirement), "PopulationInAreaPercent(Test Area, 40%)")
